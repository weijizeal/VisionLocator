...
from detect_attribute import DetectAttribute
...
def ai_detect(label: Enum, numbers: int = 1, model='best', sort_axis: list = ['y', 'x'], sort_group: int = None, timeout: int = None) -> Union[DetectAttribute, List[DetectAttribute]]:
    time_stamp = datetime.now().strftime('%Y.%m.%d %H.%M.%S')
    temp = f'.history/.ai/temp.png'
    ...
    if check_predict == 1:
        info = __convert_ai_infos(predict, crop_list, sort_axis, sort_group)[0]
        return DetectAttribute(ai_infos=info)

    else:
        infos = __convert_ai_infos(predict, crop_list, sort_axis, sort_group)
        results = []
        for info in infos:
...
def __convert_ai_infos(predict, crop_list, sort_axis, sort_group) -> list:
    ...
    for p in predict:
        for i, (x1, y1, x2, y2), conf, crop in zip(p.boxes.cls, p.boxes.xyxy, p.boxes.conf, crop_list):
            ...
            infos.append({Label(int(i)).name: {'x': x, 'y': y, 'conf': round(float(conf), 2), 'crop': crop}})
        infos = __sort_ai_infos(infos, Label(int(i)).name, sort_axis, sort_group)
    return infos
